...
`githubmodels` and `cohere` are genuinely free, recurring-quota providers (daily/monthly reset, no card ever) — unlike most of the others in this list, which are one-time trial credits. Both are included in `FREE_AUTO_PROVIDERS`, ordered last since their quotas (50-150 req/day, 1,000 req/month respectively) are the lowest in the pool — they're reached as last-resort free fallbacks rather than on every automatic request.
